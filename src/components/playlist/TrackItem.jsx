...
const TrackItem = ({ track, index }) => {
  ...
  const highlightText = (text, query) => {
    ...
    return parts.map((p, i) => 
      p.toLowerCase() === query.toLowerCase() ? <span key={i} className="bg-purple-500/30 px-0.5 rounded">{p}</span> : p
    );
  };
  ...
  return (
    ...
    >
      <img src={track.cover} alt={track.title} className="w-12 h-12 rounded-lg object-cover shadow-md" />
      ...
      <div className="text-white/30 flex items-center gap-2">
        {isActive && isPlaying ? (
          <div className="flex gap-0.5 items-end h-4">
            {[...Array(4)].map((_, i) => <div key={i} className="visualizer-bar" style={{height: `${8 + i*4}px`}}></div>)}
          </div>
        ) : (
          <PlayCircle size={18} />
        )}
